...
class Classification_with_Clustering():
    def __init__(self, strategy: list = [1,2,3], n_components: int = 64, min_cluster_size: int = 15, batch_size: int = 1, data_generator = None, n_epochs = None):#, model: tf.keras.Model
        #super().__init__()
        """
        Parameters:
    ...
    def Train_load_DimRed_Clustering(self, inputs_train, inputs_validate, 
        ...
        if train_dimR:
            ...
            if clustering == 'pumap':
                self.embedder = self.embedder.fit(np.concatenate((inputs_train,inputs_validate),axis=0))#dim_r_embed_train = self.embedder.fit_transform(inputs_train) 
    ...
    def Test_load_DimRed_Clustering(self, inputs_validate, inputs_test, model_save_path = None, 
                                     clustering_strategy = 'hard', clustering: str = 'pumap',dim_reduction_metric: str = 'cosine',pad_len:int = 30):
...
class Classification_with_clustering_model_():
    def __init__(self, model, strategy: int = 1, batch_size: int = 1, positional_encoding: bool = False):
        super().__init__()
                #super().__init__()
        """
        Parameters:
    ...
    def get_encoder_model(self, CLS_clustered_inputs_shape, clustered_train_shape, dim_r_embed_train_shape, clustering_strategy: str, num_nodes: int = 1, intermediate_dim: int = 2048, return_training_params: bool = False, model_save_path: str = None, model_name: str = None, load: bool = False, model_load_path = None, layers_to_freeze: list = [1], num_enc_layers: int = 1, include_rnn: bool = False, rnn_name: str = 'bilstm', dropout_before_first_fnn: bool = False, dropout_before_inner_fnn: bool = False, ffn_dropout_value:float = 0.08, from_encoder: bool = False, dropout_after_first_encoder: bool = False, dropout_after_second_encoder: bool = False, encoder_dropout_value: float = 0.1,bilstm_before_encoder: bool = False, two_rnn_to_enc: bool = False, problem_type: str = 'multi_label', only_RNN: bool = False, num_heads: int = 8):
        """
        Parameters:
        ...
        if strat == 1 or strat == 2:
            ...
            if load:
                ...
                if bilstm_before_encoder == False and only_RNN == False:
                    ...
                    dense_cl = tf.keras.layers.Dense(64,activation = 'softmax')(clustered) #30 was old value                    dense_cl = tf.keras.layers.Flatten()(dense_cl)
...
class Classification_without_clustering_model_():
    def __init__(self, model, batch_size: int = 1, positional_encoding: bool = False):
        super().__init__()
                #super().__init__()
        """
        Parameters:
    ...
    def get_encoder_model(self, inputs_shape, num_nodes: int = 1, intermediate_dim: int = 2048, return_training_params: bool = False, model_save_path: str = None, model_name: str = None, load: bool = False, model_load_path = None, layers_to_freeze: list = [1], num_enc_layers: int = 1, include_rnn: bool = False, rnn_name: str = 'bilstm',dropout_before_first_fnn: bool = False,dropout_before_inner_fnn: bool = False, ffn_dropout_value:float = 0.08, from_encoder: bool = True, dropout_after_first_encoder: bool = False, dropout_after_second_encoder: bool = False, encoder_dropout_value: float = 0.1,bilstm_before_encoder: bool = False, problem_type: str = 'multi_label', only_RNN: bool = False, num_heads: int = 8):
        """
        Parameters:
            The 'inputs' to this model are assumed to be of the shape (batches, number of chunks (which can be of variables sizes),  (output dimension size from BERT))
