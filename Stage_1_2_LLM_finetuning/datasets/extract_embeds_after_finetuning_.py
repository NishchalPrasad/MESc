...
def parse_args():
    parser = argparse.ArgumentParser(description="Extracting CLS embedding step: LLM (BERT, GPT-Neo, GPT-J)")
    ...
    parser.add_argument(
        "--save_dir", 
        type=str, 
        default='LEGAL-P_E/experiments/finetuned_models/', 
    ...
    parser.add_argument(
        ...
        type=str,
        default="LEGAL-P_E/experiments/finetuned_models/",
        help="path to save the extracted CLS embeddings"
    )
    parser.add_argument(
        "--which_split",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to data to use for extraction."
    )

    parser.add_argument(
        "--incorporate_zero_padding",
        type=bool,
        default=False,
        help="""We used 0 post padding while training the gpt models. 
        If you do not want to incorporate this into your current replication (for stages 3 and 4 for MESc) you can choose to set this variable to 'True'. 
        Do keep in mind that the model (we released) were trained with this padding. 
        So when using those models and set this varible here to True/False and check for the performance of your learning models for stage 3 and 4.
        For extraction (since there is no training) this does not affect the weights of the model."""
    )
    ...
    if args.dataset_subset in ["ildc"]:
        print("""since the max allowed run time in our server was 20 hours and parsing through ILDC (with A100 80GB GPU) takes more 
              than 20 hours, we split the dataset and parse in three steps. The step number can be found in the output and also in 
              the file name. Please use that number in the argument input (--which_split) for parsing the remaining data. The first 
              pass of this script for ILDC will inherently parse through the development and test dataset (you can switch thet off), 
              so you need not pass those arguments for the remaining splits.""")
        print(f"\n\nParsing split {args.which_split} of 3 for {args.dataset_subset}'s train set.\n\n")
...
def grouped_input_ids(
    ...
    incorporate_zero_padding = False
):
    tokenised_chunks = []
    seq_lengths = []
    left = 0
    ...
    if base_model in ["default","bert"]:
        CLS = tokenizer.cls_token
        SEP = tokenizer.sep_token
        pad_value = 0
    elif base_model in ["gpt"]:
...
def get_output_for_one_vec(
    ...
    last_token = seq_lengths if base_model in ["bert"] else last_token
    if combine_layers:
        vec = []
        ...
        vec = encoded_layers[1][num_layers][0][seq_lengths-last_token]
        vec = vec.detach().cpu().numpy()
        return vec
...
def generate_training_data(
    ...
    for i in tqdm(range(len(df[text_column]))):
        ...
        _input_ids, _attention_masks, seq_lengths, index = grouped_input_ids(
            tokens, 
            tokenizer, 
            length = length, 
            overlap = overlap, 
        ...
        for index,(ii, seq_len) in enumerate(zip(_input_ids,seq_lengths)):
            vec = get_output_for_one_vec(
                model,
                device,
                ii,
                _attention_masks[index],
                seq_lengths=seq_len,
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if args.dataset_subset in ["eurlex","ecthr_a","ecthr_b","scotus"]:
        ...
        remove_first = True
        #data = pd.read_csv(r"LEGAL-PE/experiments/datasets/ILDC/ILDC_multi.csv")
        data = pd.read_csv(args.data_source+"/ILDC_multi.csv")
        data_s = pd.read_csv(args.data_source+"/ILDC_single.csv")
    ...
    if args.hggfc_model_name in ["nlpaueb/legal-bert-base-uncased","law-ai/InLegalBERT","google-bert/bert-base-uncased"]:
        base_model = "bert"
    elif args.hggfc_model_name in ["EleutherAI/gpt-neo-1.3B","EleutherAI/gpt-neo-2.7B","EleutherAI/gpt-j-6b"]:
        base_model = "gpt"
    else:
        base_model = "default"
    ...
    if args.extracting_from_ft:
        if args.trained_without_accelerate:
            checkpoint = torch.load(args.loading_model_path)['model']
    ...
    if args.get_train_data:
        trainData, train_labels, train_indexes = generate_training_data(
            train_data,
            model,
            device,
            tokenizer, 
            ...
            base_model = base_model,
            incorporate_zero_padding = args.incorporate_zero_padding
        )
    ...
    if args.get_validation_data:
        validationData, validation_labels, validation_indexes = generate_training_data(
            validation_data,
            model,
            device,
            tokenizer, 
    ...
    if args.get_test_data:
        testData, test_labels, test_indexes = generate_training_data(
            test_data,
            model,
            device,
            tokenizer, 
